	  ...
	  public:
		...
		{ // timeout in milliseconds
			...
			if (timeout != 0) 
			{
				...
				FD_ZERO(&wfds);
				FD_SET(sockfd, &rfds);
				if (txbuf1.size()) { FD_SET(sockfd, &wfds); }
			...
			while (1)
			{
				int N = rxbuf.size();
				rxbuf.resize(N + 1500);
				ssize_t ret = recv(sockfd, (char*)&rxbuf[0] + N, 1500, 0);
				int err = WSAGetLastError();
				if (ret < 0 && (err == WSAEINPROGRESS || err == WSAEWOULDBLOCK))
				{
					rxbuf.resize(N);;
					break;
				}
		...
		void close() 
		{
			...
			txbuf1.insert(txbuf1.end(), header.begin(), header.end());
		}
	};
...
namespace easywsclient 
{
	...
	{
		string surlA = g_iphost;
	
		return ::from_url(surlA, true, origin);
	}
